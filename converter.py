...
def sizeof(text: str, font: ImageFont.ImageFont = base_font) -> tuple[int, int]:
    """
    Gets the size of the text when rendered in the given font in pixels

    Parameters
    ----------
    text: str
        The input text to render
    font: ImageFont.ImageFont, optional, default: base_font
    ...
    _, _, *size = draw.textbbox((0, 0), text, font)
    return size


def ascii_to_image(text: str, font: ImageFont.ImageFont = base_font) -> Image.Image:
    """
    Converts the given text to an image using the given font
...
def get_brightness_of_char(char: str, font: ImageFont.ImageFont = base_font) -> int:
    """
    Gets the brightness of the given character when rendered in the given font
